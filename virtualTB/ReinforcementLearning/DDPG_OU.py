...
print("batch_size length: ", len(batch_size))
...
for i in range(len(gamma)):
    ...
    tmp_path = "./DDPG_model01/"

    for j in range(7):
        new_logger = configure(tmp_path, ["csv", "tensorboard"],
                               log_suffix="_gamma={}_batch_size={}_noise=OU_{}".format(gamma[i], batch_size[i], j))
        ...
        #                                  sigma=0.25 * np.ones(n_actions))  # sigma = sigma[i]
        ...
        if mean_reward_max <= mean_reward:
            ...
            parameter.append(batch_size[i])

        print("mean_reward: ", mean_reward)
        print("std_reward: ", std_reward)
        model.save("./DDPG_model01/gamma={}_reward_scale={}_noise=OU_{}.pkl".format(gamma[i], batch_size[i], j))
